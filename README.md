...
- 🔍 **快速查询**: 输入即弹**补全下拉**（前缀 + 模糊包含匹配，↑↓ 选择、回车/点击即查）+ 大小写不敏感查找 + `@@@LINK=` 跳转自动解析
...
- 🔊 **发音功能**: 一键朗读当前词 + 词条内发音图标可点击
  ...
  - 支持 ⌘P 快捷键
- 🕒 **查询历史**: 自动记录 + 右侧栏**按日期分组**（今天/昨天/7 天内/更早）+ **搜索过滤** + 点击回查 + 右键删除 + 清空需确认
- 💡 **加载与提示**: 索引构建进度条 + 构建失败汇总；未找到时显示**"您是否想查"点击建议**；查词异常不崩溃、明确提示
...
- `./dictionaries/`（**内置词库**：5 本词典已放在这里，CI 打包时一并打入安装包，离线开箱即用）
- 当前工作目录（兼容旧习惯，也支持用户自行放置）
- 打包版（PyInstaller）：额外扫描 `_internal/dictionaries`（内置）→ 用户数据目录 `dictionaries/`（扩充，见 `funlex/core/paths.py`）
- 也可在"设置"中添加更多路径
