...
- 🎨 **Liquid Glass 主题**: Minimalism 2.0 风格——半透明玻璃面板、柔和渐变背景、系统蓝点缀（可扩展深色主题）
- 🎨 **双视图模式**:
  - 默认视图：单栏完整显示（保留原排版）
  - 等分视图：按词性拆分为等宽面板（QSplitter）
- 🔊 **发音功能**: 一键朗读当前词（系统 TTS，离线可用；支持 ⌘P 快捷键）
- 🕒 **查询历史**: 自动记录 + 右侧栏点击回查 + 一键清空
- ⚙️ **可配置**: 默认词典 / 词典路径 / 视图模式 / 历史条数 / 字号 / 字体（`config.json`）
...
> 启动时默认选中词条数最多的词典（通常是牛津第10版），可在"设置"中修改默认词典。
...
│       ├── entry_view_split.py  # 等分视图（按词性拆分）
