...
- [x] P3.4 历史记录侧边栏 UI 增强
  - QTreeWidget 按日期分组（今天/昨天/7天内/更早，组头含条数可展开，无词条 data 不可误点）
  - 顶部搜索框按词过滤（过滤时扁平显示）；每条显示相对时间 + 词典/完整时间工具提示；清空历史需确认
...
- [x] P3.7 加载状态与错误提示优化
  - 状态栏右侧细进度条显示索引构建进度，完成自动隐藏
  - 构建失败经 dict_failed 信号上报，最终状态汇总"N 本词典构建失败"
  - 查词异常容错（lookup 抛错 → 未找到页 + 状态提示，不崩溃）
  - 未找到页附带"您是否想查"点击建议（前缀/模糊，复用 funlex://phrase 跳转）
